...
namespace One_Man_Army
{
    ...
    public abstract class ParticleSystem
    {
        ...
        protected ExplosionParticle[] particles;
        ...
        protected Queue<ExplosionParticle> freeParticles;
        ...
        protected float minScaleAddend = 0f;
        protected float maxScaleAddend = 0f;
        ...
        public virtual void Initialize()
        {
            ...
            particles = new ExplosionParticle[howManyEffects * maxNumParticles];
            freeParticles = new Queue<ExplosionParticle>(howManyEffects * maxNumParticles);
            for (int i = 0; i < particles.Length; i++)
            {
                particles[i] = new ExplosionParticle();
        ...
        protected virtual void LoadContent()
        {
            ...
            {
                // load the texture....
                texture = game.Content.Load<Texture2D>("Sprites/Particles/" + textureFilename);
            ...
            {
                // Otherwise, set the origin to the default values of 64, 64. 
                // All textures should be 128, 128 in size anyway.
                origin.X = 64;
                origin.Y = 64;
            }
        }
        ...
        public virtual void AddParticles(Vector2 where, float radius)
        {
            float scale = radius / 128;
            scale = (float)Math.Pow(scale, .8);

            // the number of particles we want for this effect is a random number
            // somewhere between the two constants specified by the subclasses.
            int numParticles = (int)(random.Next(minNumParticles, maxNumParticles) * scale);
            ...
            for (int i = 0; i < numParticles && freeParticles.Count > 0; i++)
            {
                ...
                InitializeParticle(p, where, scale);               
            }
        }
        ...
        /// </param>
        protected virtual void InitializeParticle(ExplosionParticle p, Vector2 where, float scalar)
        {
            ...
            float velocity =
                One_Man_Army_Game.RandomBetween(minInitialSpeed, maxInitialSpeed) * scalar;
            float acceleration =
                One_Man_Army_Game.RandomBetween(minAcceleration, maxAcceleration) * scalar;
            float lifetime =
                One_Man_Army_Game.RandomBetween(minLifetime, maxLifetime) / 2 + 
                One_Man_Army_Game.RandomBetween(minLifetime, maxLifetime) / 2 * scalar;
            float scale =
                One_Man_Army_Game.RandomBetween(minScale, maxScale) * scalar;
            float rotationSpeed =
                One_Man_Army_Game.RandomBetween(minRotationSpeed, maxRotationSpeed);
            float scaleAddend =
                One_Man_Army_Game.RandomBetween(minScaleAddend, maxScaleAddend);

            // then initialize it with those random values. initialize will save those,
            // and make sure it is marked as active.
            p.Initialize(
                where, velocity * direction, acceleration * direction,
                lifetime, scale, scaleAddend, rotationSpeed, texture);
        }
        ...
        {
            float angle = One_Man_Army_Game.RandomBetween(0, MathHelper.TwoPi);
